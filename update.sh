...
IFS=' ' read -ra update_variants <<<"$(get_variants . $(get_arch) "${2-}") $(get_variants . windows-$(get_arch) "${2-}")"
...
'
    ...
    for key_type in "node" "yarn"; do
      while read -r line; do
        ...
        if [[ "$variant" == "" ]] || [[ ! "$(get_windows_variants)" =~ $variant ]]; then
          replacement="\\1${key}${new_line}\\1\\2"
        else
          replacement="\\1'${key}',${new_line}\\1\\2"
        fi

        sed -Ei -e "s/([ \\t]*)($pattern)/${replacement}/" "${dockerfile}-tmp"
      done <"keys/${key_type}.keys"
      sed -Ei -e "s/'${key}',/'${key}'/" "${dockerfile}-tmp"
...
echo '#### DO NOT MODIFY. THIS FILE IS AUTOGENERATED ####
' | cat - appveyor.yml.template >appveyor.yml
...
for version in "${versions[@]}"; do
  ...
  if [ -f "${version}/Dockerfile" ]; then
    add_travis_stage "${version}" "default"
  ...
  for variant in "${variants[@]}"; do
    ...
    add_travis_stage "${version}" "${variant}"
  ...
  IFS=' ' read -ra windows_variants <<<"$(get_windows_variants "$parentpath")"
  if [ ${#windows_variants[@]} -gt 0 ]; then
    for variant in "${windows_variants[@]}"; do
      add_appveyor_stage "${version}" "${variant}"

      update_variant=$(in_variants_to_update "${variant}")

      if [ "${update_version}" -eq 0 ] && [ "${update_variant}" -eq 0 ]; then
        update_node_version "${baseuri}" "${versionnum}" "${parentpath}/Dockerfile-${variant}.template" "${version}/windows/${variant}/Dockerfile" "${variant}" &
      fi
    done
  fi
done
